(function (root) {
  ...
  function seed(options = {}) {
    ...
    addOnce(orderStore.receipts, { id: ids.receipt, supplierOrderId: ids.supplierOrder, requisitionId: ids.clientOrder, date: day, items: [{ orderLineId: ids.line, quantity, quantityAccepted: quantity, quantityRejected: 0, lot: 'LOTE-DEMO-01', serial: null, expiryDate: null }], type: 'total', user: 'José Velasco', observations: 'Recepción total validada contra OC del proveedor.', reference: ids.supplierInvoice, warehouseId: '1', documentType: 'supplier_remission', qualityStatus: 'released', evidence: [{ id: `EVD-${ids.receipt}`, name: 'remision-proveedor-demostracion.pdf', type: 'application/pdf', hash: `DEMO-RECEIPT-EVIDENCE-${code}` }] });
    ...
    addOnce(orderStore.shipments, { id: ids.shipment, folio: ids.shipment, date: day, requisitionId: ids.clientOrder, items: [{ orderLineId: ids.line, quantity }], deliveredAt: at('17:05'), type: 'total', status: 'delivered', recipient: 'Ana Martínez', evidence: [{ id: `EVD-${ids.shipment}`, name: 'remision-cliente-demostracion.pdf', type: 'application/pdf', hash: `DEMO-DELIVERY-EVIDENCE-${code}` }], observations: 'Entrega total confirmada al cliente.' });
  ...
  // El caso demostrativo se conserva para capacitación, pero no se inserta al iniciar.
