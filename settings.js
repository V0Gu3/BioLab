(function () {
  ...
  function openUserDialog(userId) {
    ...
    let password = q('#systemUserPassword'); if (!password) { const label = document.createElement('label'); label.className = 'field wide'; label.innerHTML = '<span>Contraseña de acceso <em>opcional al editar</em></span><input id="systemUserPassword" type="password" minlength="12" autocomplete="new-password" placeholder="Mínimo 12 caracteres" /><small>Se guarda con hash y nunca se muestra de nuevo.</small>'; q('#systemUserForm .form-grid').append(label); password = q('#systemUserPassword'); } password.required = !user;
