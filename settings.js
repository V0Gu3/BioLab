(function () {
  ...
  function openUserDialog(userId) {
    ...
    if (!password) {
      const section = document.createElement('section'); section.id = 'systemUserPasswordManagement'; section.className = 'user-password-management wide';
      section.innerHTML = '<div class="user-password-head"><div><strong>Contraseña de acceso</strong><small id="systemUserPasswordHelp"></small></div><button id="systemUserPasswordReset" type="button" class="filter" hidden><i data-lucide="key-round"></i>Restablecer contraseña</button></div><div id="systemUserPasswordFields" class="user-password-fields"><label class="field"><span>Nueva contraseña</span><input id="systemUserPassword" type="password" minlength="12" autocomplete="new-password" placeholder="Mínimo 12 caracteres" /></label><label class="field"><span>Confirmar contraseña</span><input id="systemUserPasswordConfirm" type="password" minlength="12" autocomplete="new-password" placeholder="Repite la contraseña" /></label></div>';
      q('#systemUserForm .form-grid').append(section); password = q('#systemUserPassword');
      q('#systemUserPasswordReset').addEventListener('click', () => { q('#systemUserPasswordFields').hidden = false; password.required = true; q('#systemUserPasswordConfirm').required = true; q('#systemUserPasswordHelp').textContent = 'La nueva contraseña sustituirá la anterior y no podrá consultarse después.'; password.focus(); });
    }
    q('#systemUserForm').reset(); q('#systemUserPasswordFields').hidden = Boolean(user); q('#systemUserPasswordReset').hidden = !user; q('#systemUserPasswordHelp').textContent = user ? 'La contraseña actual nunca se muestra. Puedes asignar una nueva si el usuario perdió el acceso.' : 'Define una contraseña de al menos 12 caracteres para el nuevo usuario.'; password.required = !user; q('#systemUserPasswordConfirm').required = !user;
